...
args <- commandArgs(T)
MODEL_RESULTS <- args[1]
CLUSTER_NAMES <- args[2]
CELL_TYPE_DECODING <- args[3] # roadmap_cell_types.txt

DROP_TRAITS <- c()

models <- read.table(MODEL_RESULTS, head = T, sep = '\t')
...
for(cluster in unique(models$cluster)) {
  models$coef[models$cluster==cluster] <- models$coef[models$cluster==cluster] / max(models$coef[models$cluster==cluster])
}

models <- left_join(models, tissue_mappings %>% dplyr::select(cell_type, tissue))
duplicates <- models[,c('cell_type', 'tissue')] %>%
  unique() %>%
  dplyr::group_by(tissue) %>%
  dplyr::summarize(count=n()) %>%
  dplyr::filter(count>1)
duplicated_tissues <- duplicates$tissue
for(i in glue('Duplicated tissue: {duplicated_tissues} (dropping)')) {
  warning(i)
}
models <- models[!models$tissue %in% duplicated_tissues,]
...
models <- left_join(models, cluster_names) %>%
  dplyr::select(new_name, cell_type, pvalue, coef, tissue) %>%
...
p <- ggplot(for_heatmap_figure) +
  ...
  xlab('Our cell type')
pdf('enhancer-similarity-heatmap.pdf', height=3, width=5)
p
dev.off()
