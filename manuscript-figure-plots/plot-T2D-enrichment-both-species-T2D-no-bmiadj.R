...
#human_results <- bind_rows(lapply(unlist(strsplit(HUMAN_MODELS, ',')), load_result_file))
human_results <- bind_rows(lapply(HUMAN_MODELS, load_result_file))
human_results$species <- 'human'
#rat_results <- bind_rows(lapply(unlist(strsplit(RAT_MODELS, ',')), load_result_file))
...
p <- ggplot(results) +
  ...
  #geom_text(aes(x = Category, label='*', color=species, y=Coefficient+3*Coefficient_std_error), show.legend=F, data=results[results$bonferroni_significant,], position=position_dodge(0.9)) +
